...
export async function getSubscribeEnquiryTableData(filter) {
  // Fake delay
  ...
  try {
   const response = await fetch(
        process.env.NEXT_PUBLIC_ADMIN_API_URL + "api/subscribeenquiry?limit="+filter.limit+"&skip="+filter.page,
        {
          next: { revalidate: 60 }
        }
      );
