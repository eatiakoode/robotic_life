...
import { addColorAPI } from "@/api/color";
...
const CreateList = () => {
  ...
  const addColor = async (e) => {
    ...
    try {
      const data = await addColorAPI(title);

      toast.success(data.message || "Color added successfully!");
  ...
  return (
    <>
      <form onSubmit={addColor} className="row">
        <div className="col-lg-6 col-xl-6">
          <div className="my_profile_setting_input form-group">
            <label htmlFor="colorTitle">Color Title</label>
            <input type="text" className="form-control" id="colorTitle" value={title} onChange={handleTitleChange} />
