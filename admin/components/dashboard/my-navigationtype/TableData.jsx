...
import { deleteNavigationTypeAPI } from "../../../api/navigationtype";
import { useState } from "react";
import { useRouter } from "next/navigation";
import { toast } from 'react-toastify';

const TableData = ({ navigationTypes = [], loading = false, error = null, onRefresh }) => {
  ...
  const deleteNavigationType = async (id) => {
    ...
    try {
      ...
      toast.error("Failed to delete Navigation Type.");
    }
  };
  ...
  if (loading) {
    return (
      <div className="text-center py-5">
        ...
        <p className="mt-2">Loading Navigation Types...</p>
      </div>
    );
  }
  ...
  // Show empty state
  if (!navigationTypes || navigationTypes.length === 0) {
    return (
      <div className="text-center py-5">
        <div className="alert alert-info" role="alert">
          <i className="fas fa-info-circle me-2"></i>
          No Navigation Types found.
        </div>
      </div>
    );
  }
  ...
  let tbodyContent = navigationTypes.map((item) => (
      ...
      <td className="align-middle text-center">
        <ul className="view_edit_delete_list mb0 d-flex justify-content-center">
          <li className="list-inline-item" title="Edit">
            <button onClick={() => router.push(`/cmsthebotsworld/edit-navigationtype/${item._id}`)}>
