...
import { addPowerSourceAPI } from "@/api/powersource";
...
const CreateList = () => {
  ...
  const addPowerSource = async (e) => {
    ...
    try {
      const data = await addPowerSourceAPI(title);

      toast.success(data.message || "Power Source added successfully!");

      setTimeout(() => {
        router.push("/cmsroboticlife/my-powersource");
      }, 1000);
  ...
  return (
    <>
      <form onSubmit={addPowerSource} className="row">
        <div className="col-lg-6 col-xl-6">
          <div className="my_profile_setting_input form-group">
            <label htmlFor="powerSourceTitle">Power Source Title</label>
            <input type="text" className="form-control" id="powerSourceTitle" value={title} onChange={handleTitleChange} />
