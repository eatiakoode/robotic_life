...
const CreateList = () => {
  ...
  const [navigationType, setNavigationType] = useState({ name: "", status: false, description: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // }
      const data = await updateNavigationTypeAPI(id, title);
      // alert("Manufacturer updated successfully!");
      // router.push("/cmswegrow/my-manufacturer");
      toast.success(data.message);
      if (data.status == "success") {
        setTimeout(() => {
          router.push("/cmswegrow/my-navigationtype");
        }, 1500);
      }
    } catch (error) {
      alert("Failed to update Navigation Type.");
      console.error(error);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="row">
        ...
        <div className="col-xl-12">
          <div className="my_profile_setting_input">
            <button className="btn btn1 float-start" type="button" onClick={() => window.location.href = '/cmswegrow/my-navigationtype'}>Back</button>
