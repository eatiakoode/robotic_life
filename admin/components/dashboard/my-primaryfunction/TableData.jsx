...
import { deletePrimaryFunctionAPI } from "../../../api/primaryfunction";
import { useState } from "react";
import { useRouter } from "next/navigation";
import { toast } from 'react-toastify';

const TableData = ({ primaryFunctions = [], loading = false, error = null, onRefresh }) => {
  ...
  const deletePrimaryFunction = async (id) => {
    ...
    try {
      ...
      toast.error("Failed to delete Primary Function.");
    }
  };
  ...
  if (loading) {
    return (
      <div className="text-center py-5">
        ...
        <p className="mt-2">Loading Primary Functions...</p>
      </div>
    );
  }
  ...
  // Show empty state
  if (!primaryFunctions || primaryFunctions.length === 0) {
    return (
      <div className="text-center py-5">
        <div className="alert alert-info" role="alert">
          <i className="fas fa-info-circle me-2"></i>
          No Primary Functions found.
        </div>
      </div>
    );
  }
  ...
  let tbodyContent = primaryFunctions.map((item) => (
      ...
      <td className="align-middle text-center">
        <ul className="view_edit_delete_list mb0 d-flex justify-content-center">
          <li className="list-inline-item" title="Edit">
            <button onClick={() => router.push(`/cmsthebotsworld/edit-primaryfunction/${item._id}`)}>
