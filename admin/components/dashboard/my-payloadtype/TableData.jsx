...
import { deletePayloadTypeAPI } from "../../../api/payloadtype";
import { useState } from "react";
import { useRouter } from "next/navigation";
import { toast } from 'react-toastify';

const TableData = ({ payloadTypes = [], loading = false, error = null, onRefresh }) => {
  ...
  const deletePayloadType = async (id) => {
    ...
    try {
      ...
      toast.error("Failed to delete Payload Type.");
    }
  };
  ...
  if (loading) {
    return (
      <div className="text-center py-5">
        ...
        <p className="mt-2">Loading Payload Types...</p>
      </div>
    );
  }
  ...
  let tbodyContent = payloadTypes.map((item) => (
      ...
      <td className="align-middle text-center">
        <ul className="view_edit_delete_list mb0 d-flex justify-content-center">
          <li className="list-inline-item" title="Edit">
            <button onClick={() => router.push(`/cmsthebotsworld/edit-payloadtype/${item._id}`)}>
